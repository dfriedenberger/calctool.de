...
public class Subtraction extends MathFunction
{
    ...
    public MathTerm eval(MathEvaluator evaluator,MathTerm[] term)
    {
        ...
        if(evaluator.isNumber(term[1]) && term[1].getNumber().isNumber(0)) return new MathTerm(term[0]);
        ...
        if(evaluator.isNumber(term[0]) && evaluator.isNumber(term[1]))
        {
            ...
            if((n0.isFractal() && n1.isLong())
               || (n0.isLong() && n1.isFractal())
               || (n0.isFractal() && n1.isFractal()))
            {
                MathNumber n = new MathNumber(n0.getNumerator() * n1.getDemoninator() - n0.getDemoninator() * n1.getNumerator()
        ...
        //code copied from Addition "+" --> "-"
        if(term[0].isMatrix() && term[1].isMatrix())
        {
        	//dimensions fit?
        	if((term[0].getMatrix().rows() == term[1].getMatrix().rows()) && (term[0].getMatrix().columns() == term[1].getMatrix().columns()))
   			{
	        	...
	        	for(int i = 0;i < term[1].getMatrix().rows();i++)
	        	{
	        		...
	        		{
	        			//do sutraction
	        			MathTerm MCellRes =	evaluator.eval("-",new MathTerm[]{ term[0].getMatrix().getCell(i, j) , term[1].getMatrix().getCell(i,j) } );
